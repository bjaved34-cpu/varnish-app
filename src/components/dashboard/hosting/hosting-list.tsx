...
export function HostingList() {
    ...
    const [selectedHosting, setSelectedHosting] = React.useState<number[]>([]);

    const filteredHosting = hosting.filter(hosting =>
        hosting.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        hosting.status.toLowerCase().includes(searchQuery.toLowerCase())
    );

    const toggleAll = () => {
        if (selectedHosting.length === filteredHosting.length && filteredHosting.length > 0) {
            setSelectedHosting([]);
        } else {
            setSelectedHosting(filteredHosting.map(d => d.id));
        }
    };
    ...
    const isAllSelected = filteredHosting.length > 0 && selectedHosting.length === filteredHosting.length;
    ...
    return (
        <div className="w-full bg-[#F6F6F6] rounded-[10px] p-[10px] md:h-auto flex flex-col border border-[#D8D8D8]">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 md:mb-2 px-2">
                <h2 className="text-[15px] font-semibold text-[#8899aa]">List of Hosting</h2>
            ...
            <div className="flex-1 overflow-x-auto bg-white rounded-lg border border-[#E8E8E8] min-h-[420px]">
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {filteredHosting.map((hosting) => {
                                const isSelected = selectedHosting.includes(hosting.id);
                                return (
                                    <TableRow key={hosting.id} className={cn("border-[#E8E8E8] hover:bg-gray-50/50 h-[64px]", isSelected && "bg-gray-50/80")}>
                                        <TableCell className="text-center">
                                            <button
                                                onClick={() => toggleRow(hosting.id)}
                                                className={cn(
                                                    "w-4 h-4 rounded border flex items-center justify-center mx-auto transition-colors",
                                        ...
                                        <TableCell className="text-[#1a2332] text-xs font-medium">{hosting.name}</TableCell>
                                        <TableCell className="text-[#1a2332] text-xs font-bold">{hosting.cdn}</TableCell>
                                        <TableCell className="text-[#1a2332] text-xs font-bold">{hosting.bandwidth}</TableCell>
                                        <TableCell className="text-[#1a2332] text-xs font-bold">{hosting.cache}</TableCell>
                                        <TableCell className="text-[#1a2332] text-xs font-bold">{hosting.expiry}</TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-1.5 px-2 py-1 bg-[#F0FDF4] text-[#166534] rounded-md border border-[#DCFCE7] w-fit">
                                                ...
                                                <span className="text-[10px] font-semibold">{hosting.status}</span>
                                            </div>
                                        </TableCell>
                ...
                <div className="mt-auto px-4 py-4 flex items-center justify-between border-t border-[#E8E8E8] bg-[#fcfcfc]">
                    <div className="text-xs text-[#8899aa] font-medium">
                        Showing 1 to {filteredHosting.length} of {filteredHosting.length} entries
                    </div>
